...
fn gen_configs(client: &Path, socket: &Path, out: &Path, tuning: &UdfTuning) -> anyhow::Result<()> {
    ...
    let mut xml = String::from(
        "<!-- Generated by `model-bridge gen-configs`; the next run overwrites\n\
         this file, so retune through the `gen-configs` flags (pool-size,\n\
         command-read-timeout, max-command-execution-time), not by editing\n\
         here. Flag names are spelled without dashes above because an XML\n\
         comment must not contain two consecutive dashes.\n\n\
         Settings that differ from the ClickHouse defaults:\n\
         - `deterministic`: outputs are stable for a fixed model revision\n\
           (passports pin revisions), which permits these functions in\n\
           `ALTER TABLE ... UPDATE` backfills on `Replicated*` tables, where\n\
           non-deterministic functions are rejected.\n\
         - `command_read_timeout` (ms) and `max_command_execution_time` (s):\n\
           the client replies only after the daemon finishes the whole batch,\n\
           and a pool slot is held for that long; the 10-second defaults are\n\
           too tight for large blocks on heavy models.\n\
         - `modelEvaluate` takes `Array(Float64)` because ClickHouse float\n\
           expressions produce `Float64` and an implicit narrowing cast is\n\
           refused; the client narrows to the model's native float32.\n-->\n\
         <functions>\n",
    );
